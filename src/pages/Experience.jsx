            ...
            <div className='container mx-auto px-6 relative z-10'>
                ...
                <div className='max-w-3xl mb-16'>
                    ...
                    <h2 className='text-4xl md:text-5xl font-bold mt-4 mb-6 animate-fade-in animate-delay-100 text-accentBlue'>
                        Expeience that <span className='font-heading italic font-normal text-whitePrimary'>speaks volumes.</span></h2>
                        ...
                        {experiences.map((exp, idx) => (
                            <div key={idx} className='relative grid md:grid-cols-2 gap-8 animate-fade-in'
                                ...
                                <div className='absolute left-0 md:left-1/2 top-0 w-3 h-3 bg-accentBlue rounded-full -translate-x-1/2
                                    ring-4 ring-blackPrimary z-10'>
                                {exp.current && <span className='absolute inset-0 rounded-full bg-accentBlue animate-ping opacity-75' />}
                                </div>
                                
                                {/* CONTENT */}
                                <div className={`pl-8 md:pl-0 ${idx % 2 === 0 ? "md:pr-16 md:text-right" : "md:col-start-2 md:pl-16"}`}>
                                    <div className={`glass p-6 rounded-2xl border border-accentBlue/30 hover:border-accentBlue/50 transition-all duration-300`}>
                                        
                                        <span className='text-sm text-accentBlue font-medium'>{exp.period}</span>
                                        <h3 className='text-xl font-semibold mt-2'>{exp.role}</h3>
                                        <p className='text-whiteSecondary'>{exp.company}</p>
                                        <p className='text-sm text-whiteSecondary mt-4'>{exp.description}</p>
                                        <div className={`flex flex-wrap gap-2 mt-4 ${idx % 2 === 0 ? "md:justify-end" : ""}`}>
                                            {exp.technologies.map((tech, techIdx) => (
                                                <span key={techIdx} className='px-3 py-1 bg-blackSecondary text-xs rounded-full text-whiteSecondary'>{tech}</span>
                                            ))}
                                        </div>
                                    </div>
                                </div>
